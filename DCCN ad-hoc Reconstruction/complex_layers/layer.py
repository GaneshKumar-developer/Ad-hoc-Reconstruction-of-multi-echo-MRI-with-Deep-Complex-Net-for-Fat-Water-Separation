...
'COMPLEX CONVOLUTION 2D'
class complex_Conv2D_Linear(tf.keras.layers.Layer):
    def __init__(self, 
                filters = 32,
                kernel_size = (3, 3), 
                strides = (2, 2), 
                ...
                kernel_initializer = 'glorot_uniform',
                bias_initializer   = 'zeros'):
        super(complex_Conv2D_Linear, self).__init__()
        self.filters = filters
        self.kernel_size = kernel_size
        self.strides     = strides
        self.padding     = padding
    ...
    def build (self, inputs_shape):
        self.real_Conv2D_linear = tf.keras.layers.Conv2D(
            filters = self.filters,
            kernel_size = self.kernel_size, 
            strides = self.strides,
            padding = self.padding,
            ...
            kernel_initializer = self.kernel_initializer,
            bias_initializer = self.bias_initializer) 
        self.imag_Conv2D_linear = tf.keras.layers.Conv2D(
            filters = self.filters,
            kernel_size = self.kernel_size, 
            strides = self.strides,
            padding = self.padding,
            ...
            kernel_initializer = self.kernel_initializer,
            bias_initializer = self.bias_initializer) 
        super(complex_Conv2D_Linear, self).build(inputs_shape)

        
    def call(self, real_inputs, imag_inputs):
        real_outputs = self.real_Conv2D_linear(real_inputs) - self.imag_Conv2D_linear(imag_inputs)
        imag_outputs = self.imag_Conv2D_linear(real_inputs) + self.real_Conv2D_linear(imag_inputs)
        return real_outputs, imag_outputs
...
class complex_UpSampling(tf.keras.layers.Layer): 
    def __init__(self, pool_size = (2, 2), strides = (1, 1), padding = "same"):
        super(complex_UpSampling, self).__init__()
        self.pool_size = pool_size
        self.strides   = strides
        self.padding   = padding

    def build (self, inputs_shape):
        self.real_UpSampling = tf.keras.layers.UpSampling2D(size = self.pool_size)
        self.imag_UpSampling = tf.keras.layers.UpSampling2D(size = self.pool_size)
        super(complex_UpSampling, self).build(inputs_shape)
