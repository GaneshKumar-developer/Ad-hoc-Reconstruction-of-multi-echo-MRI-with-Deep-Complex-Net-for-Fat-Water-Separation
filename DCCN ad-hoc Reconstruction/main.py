...
#Normalize the magnitude part of input MRI between 0 to 1
...
batch_no = "DCCN"  #Starts from Zero
...
#==========compile/train/test==========
model.compile(optimizer=Adam(learning_rate=(0.0006)), loss=custom_loss3,run_eagerly=True)

filepath = "batch_"+str(batch_no)+"saved_weights.hdf5"

callbacks = [ReduceLROnPlateau(monitor="loss",factor=0.3, patience=10, min_lr=(0.0001), verbose=1), ModelCheckpoint(filepath,monitor="loss", verbose=1, save_best_only=True, save_weights_only=True, mode='auto') ]

start = time.time()
history = model.fit([input_train_tm,input_train_tp, dfat_train_t, te_train_t], callbacks=callbacks, batch_size=10, verbose=1, epochs=5000,shuffle=True)
end = time.time()
